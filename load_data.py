...
from torch_geometric.datasets import BAMultiShapesDataset, MoleculeNet, TUDataset
...
def convert_atoms_to_onehot(x, atom_to_idx, num_atom_types):
    ...
    atom_indices = torch.tensor(
        [atom_to_idx.get(atom_num.item(), -1) for atom_num in atomic_numbers]
    )
    ...
    if valid_mask.any():
        one_hot[valid_mask] = F.one_hot(
            atom_indices[valid_mask], num_classes=num_atom_types
        ).float()
    return one_hot
...
def load_data(name, seed=42):
    ...
    if name == "IMDB-BINARY":
        dataset = TUDataset(
            root="./data", name="IMDB-BINARY", transform=DegreeFeatures()
        )
    elif name == "reddit_threads":
        dataset = TUDataset(
            root="./data", name="reddit_threads", transform=DegreeFeatures()
        )
    elif name == "twitch_egos":
        dataset = TUDataset(
            root="./data", name="twitch_egos", transform=DegreeFeatures()
        )
    elif name == "github_stargazers":
        dataset = TUDataset(
            root="./data", name="github_stargazers", transform=DegreeFeatures()
        )
    elif name == "BBBP":
        ...
        from graph_learning.datasets.synthetic import (
            create_non_cograph_single_p4_dataset,
            create_cograph_dataset,
            merge_datasets,
        )
        ...
        # Note: max_degree=5 gives 5+1+1+1=8 features to match pretrained model expectations
