...
def main():
    ...
    parser.add_argument("--dataset", type=str,
                        choices=["BBBP", "Mutagenicity", "IMDB-BINARY", "NCI1", "BAMultiShapes", "reddit_threads",
                                 "twitch_egos", "github_stargazers", "SingleP4"], required=True)
    ...
    gnn_train_pred_tensor, train_y_tensor, train_x_dict, train_edge_dict, train_activations_dict, train_gnn_graph_embed = get_all_activations_graph(
        train_loader, model, device)
    gnn_test_pred_tensor, test_y_tensor, test_x_dict, test_edge_dict, test_activations_dict, test_gnn_graph_embed = get_all_activations_graph(
    ...
    clf, index_0_correct, index_1_correct = decision_tree_explainer(train_gnn_graph_embed, gnn_train_pred_tensor,
                                                                    test_gnn_graph_embed, gnn_test_pred_tensor,
                                                                    max_depth=1)
    tree = clf.tree_
    ...
    predicates, predicate_to_idx, predicate_node, predicate_graph_class_0, predicate_graph_class_1, rules_matrix_0, rules_matrix_1 = get_predicates_bin_one_pass(
        index_0_correct, index_1_correct, train_x_dict, train_edge_dict, train_activations_dict, val_idx, threshold,
    ...
    test_predicate_graph, test_res = get_predicate_graph(index_test, predicates, predicate_to_idx, test_x_dict,
                                                         test_edge_dict, test_activations_dict, val_idx, threshold,
                                                         use_embed=use_embed, k_hops=k_hops)
    leaf_rules_samples_0, leaf_rules_samples_1, used_predicates, clf_graph = get_discriminative_rules_with_samples(
        rules_matrix_0, rules_matrix_1, index_0_correct, index_1_correct, max_depth=args.max_depth, plot=0, text=0)
    ...
    used_alone_predicates, used_iso_predicate_node = analyze_used_predicate_nodes(predicate_node, predicate_to_idx,
                                                                                  used_predicates)
    if args.plot:
    ...
    weighted_accuracy = accuracy_score(test_y_tensor.numpy(), data_grounded_pred_array,
